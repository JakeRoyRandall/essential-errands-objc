...
int main(int argc, const char *argv[]) {
    @autoreleasepool {
        ...
        if (startingID) {
            NSUInteger selected = [tasks indexOfObjectPassingTest:^BOOL(NSDictionary *task, NSUInteger idx, BOOL *stop) { return [task[@"id"] isEqualToString:startingID]; }];
            if (selected == NSNotFound) Fail(@"--from id was not found in input");
            NSDictionary *first = tasks[selected]; [tasks removeObjectAtIndex:selected]; [tasks insertObject:first atIndex:0];
        }
